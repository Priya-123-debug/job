...
export const register = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(201)
      .json({ message: "account created successfully now", sucess: true });
  } catch (err) {
    console.log(err);
  }
};
...
export const login = async (req, res) => {
  try {
    ...
    const token = await jwt.sign(tokendata, process.env.JWT_SECERT, {
      expiresIn: "1d",
    });
    user = {
      _id: user._id,
...
export const logout = async (req, res) => {
  try {
    return res
      .status(200)
      .cookie("token", "", { maxAge: 0, httpsOnly: true, sameSite: "strict" })
      .json({
